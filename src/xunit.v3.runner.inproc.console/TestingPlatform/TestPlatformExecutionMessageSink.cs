		...
		OutputDeviceDataProducerBase("execution message sink", "fa7e6681-c892-4741-9980-724bd818f1f1"), IMessageSink, IDataProducer
{
	...
	{
		// TODO: Waiting for an answer on whether or not we need to filter out the starting message when
		// the test isn't run. https://github.com/microsoft/testfx/issues/2538#issuecomment-2323063144
		if (testMessage is ITestNotRun)
			return;
		...
		var nodeState = testMessage switch
		{
			ITestFailed failed => failed.Cause switch
			{
				...
				FailureCause.Unknown or FailureCause.Other or FailureCause.Exception or _ => new ErrorTestNodeStateProperty(new XunitException(failed)),
			},
